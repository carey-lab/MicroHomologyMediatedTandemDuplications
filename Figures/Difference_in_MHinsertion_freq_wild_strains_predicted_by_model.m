...
T = readtable([DATADIR 'MHRSumPreinGene.txt' ],'TreatAsEmpty','NA' );
...
Y  = G.N_MHPs ; 
%Y(Y>250)=250;
G.withMH_InsertFoundInNature = (ismember(G.systematic_name,N.Gene(idxN1)) | ismember(G.gene,N.Gene(idxN1)) ) ;
G.NoMH_InsertFoundInNature = (ismember(G.systematic_name,N.Gene(idxN0)) | ismember(G.gene,N.Gene(idxN0)) ) ;
...
hold on ; 
ksdensity( Y(G.withMH_InsertFoundInNature) , linspace(min(Y),max(Y),1e3) )
ksdensity( Y(G.AnyInsertFoundInNature) , linspace(min(Y),max(Y),1e3) )
lh = legend({'MTD found in nature' 'any insert'}); 
set(lh,'box','off');
xlabel('Predicted MTD score')
ylabel('Fraction of genes')
[~,p] = ttest2( Y(G.withMH_InsertFoundInNature)  , Y(G.AnyInsertFoundInNature) ) ; 
%[~,p,~] = kstest2( G.sumFloat(G.FoundInNature)  , G.sumFloat(G.AnyInsertFoundInNature) ) ; 
fc = log2(  nanmean(Y(G.withMH_InsertFoundInNature))  / nanmean(Y(G.AnyInsertFoundInNature)) ) ; 
title( sprintf('p = %0.05f fold-change = %0.04f' , p , fc) )
xlim([prctile(Y,0.1) prctile(Y,99)])
...
% 
%%
Y  = G.N_MHPs ./ G.gene_length ; 
Y  = G.sumFloat_div_geneLen ; 
fh = figure('units','centimeters','position',[5 5 8 5]);
hold on ;
ecdf( Y(G.FoundInNature) )
ecdf( Y(G.AnyInsertFoundInNature) )
ecdf( Y(~G.NoMHFoundInNature) )
xlim([prctile(Y,5) prctile(Y,90)])
legend({'Found in nature' 'Any insert' 'not no mh'},'location','best')