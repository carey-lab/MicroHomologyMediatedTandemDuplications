...
count <- type_MHkb[-match("chromosome",type_MHkb$overlap),4]
names(count) <- type_MHkb[-match("chromosome",type_MHkb$overlap),1]
ba <- barplot(count,col = "white",ylab = "MH pairs / Kilobases",space = 0.5,main = paste0(args[3],"_MH/kb"))
...
fre_fre <- log2(count/(1000*sum(type_MHkb[-match("chromosome",type_MHkb$overlap),2])/nt_wg))
names(count) <- type_MHkb[-match("chromosome",type_MHkb$overlap),1]
ba <- barplot(fre_fre,col = "white",ylab = "log2(Fre-feature/Fre-WG)",space = 0.5,main = paste0(args[3],"_log2(Fre-feature/Fre-WG)")) #,ylim = c(-0.8,0.8)
