...
args=commandArgs(T)
inter_feature <- read.table("Saccharomyces_cerevisiae.R64-1-1.dna_rm.toplevel.intersect.txt",header = F,as.is = T, fill =T)#bedtools intersect features(CDS,intron...) of each MH, file1 is pombe_n.intersect.txt
num <- table(inter_feature)

location <- read.table("Saccharomyces_cerevisiae.R64-1-1.dna_rm.toplevel.gff3.bed",header = F, comment.char = "#")#gff file2 is pombe_n.intersect
...
featurent <- nt[match(c("Essential","Non-essential"),nt[,1]),2]#nt of each feature
freWG <- feature_length[match("chromosome",as.character(total_nt$Group.1)),]*1000/WGnt


#figure of log2(MH/kb_of_feature)
frefeature <- feature_length[match(c("Essential","Non-essential"),rownames(feature_length)),]*1000/featurent
ba <- plot(4:(length(freWG)+3),frefeature[1,],type = "l",pch = 20,col = 1,ylim = c(0, 1.2*max(frefeature)), ylab = "#MH/kb_of_feature",xlab = "MH length(bp)",main = paste0("_#MH/kb_of_feature"))
...
ba <- plot(4:(length(freWG)+3),log2(frefeature[1,]),type = "l",pch = 20,col = 1,ylim = c(-2*max(log2(frefeature)),2*max(log2(frefeature))), ylab = "log2(#MH/kb_of_feature)",xlab = "MH length(bp)",main = paste0("_log2(#MH/kb_of_feature)"))
for(i in 2:dim(frefeature)[1])
{
